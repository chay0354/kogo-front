  ...
  return (
    <>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-semibold">קישורי תשלום</h2>
          <p className="text-sm text-muted-foreground">קישור אחד להרבה משלמים; המשלם מזין כרטיס חדש בעמוד המאובטח של Tranzila</p>
        </div>
              ...
              <tbody>
                {visible.map((link) => (
                  <tr key={link.id} className="hover:bg-muted/30">
                    <td>
                      <Link href={`/settings/payment-links/${link.id}`} className="font-medium text-primary hover:underline">
                        {link.title}
                      </Link>
                      {link.review_count > 0 && (
