...
/**
 * משבצות ביומן — the studio rentals as the calendar holds them, one row per
 * rental event, with the dialog that creates and edits them and the agreement
 * PDF. This is the rentals page as it was before tenants: the same list, the
 * same actions. Its title moved up to the page, and RentalDialog goes out
 * through a portal so the page's dashboard palette does not re-tint it.
 */
export default function CalendarSlotsView() {
  const { user } = useAuth();
    ...
    <div className={styles.view}>
      <div className={styles.slotsBar}>
        <p className={styles.slotsHint}>
          כל שכירות סטודיו ביומן, חד־פעמית או שבועית. חיבור שכירות לשוכר נעשה בתצוגת &quot;שוכרים&quot;.
        </p>
      ...
      <CrossFade swapKey={loading ? 'loading' : 'ready'}>
        {loading ? (
          <TableSkeleton columns={9} label="טוען שכירויות" />
        ) : items.length === 0 ? (
