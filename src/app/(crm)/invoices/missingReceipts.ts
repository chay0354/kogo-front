 ...
 * tested: what the confirmation says, when the buttons are live, and how the
 * server's answer is read back.
 */
import {
 ...
 */
export function issueConfirmationLines(count: number, nextNumber?: string): string[] {
  const what = count === 1
    ? 'תופק קבלה אחת (חשבונית מס/קבלה) לחיוב שברשימה.'
    : `יופקו ${count.toLocaleString('he-IL')} קבלות (חשבונית מס/קבלה), אחת לכל חיוב ברשימה.`;
  ...
  return [
    ...
    numbers,
    'יש לעשות זאת רק אחרי שרואה החשבון אישר את הרשימה.',
  ];
}
...
/** The outcome in sentences: what was issued, what was skipped and why, what failed. */
export function issueSummary(result: IssueMissingReceiptsResult): string[] {
  const lines: string[] = [];
  const { issued, skipped, failed } = result;
  ...
  failed.forEach((row) => lines.push(`נכשל תשלום ${row.payment_id}: ${row.error}`));
  return lines;
}
