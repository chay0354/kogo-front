...
export default function EditInstructorDialog({ instructor, isOpen, onClose: dismiss, onSave }: EditInstructorDialogProps) {
  ...
  const [formData, setFormData] = useState<InstructorFormData>({
    ...
    primary_branch: instructor.primary_branch,
    salary_model_type: instructor.salary_model_type,
    fixed_salary_per_lesson: instructor.fixed_salary_per_lesson,
    salary_tiers: instructor.salary_tiers || [],
    branch_ids: instructor.branches.map(b => b.branch.id),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error.response?.data) {
        ...
        setErrors({ general: 'שגיאה בעדכון מדריך. נסה שוב.' });
      }
    } finally {
    ...
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/50 overflow-y-auto py-8 ${dialogMotion.overlay} ${closing ? dialogMotion.overlayClosing : ''}`} onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b sticky top-0 bg-white z-10">
          <h2 className="text-xl font-bold">עריכת מדריך - {instructor.full_name}<span style={{ fontSize: '10px', color: 'white', userSelect: 'none' }}> #20</span></h2>
